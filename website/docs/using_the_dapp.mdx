...
- **Name** – 15 characters max, unique on-chain
...
Inside a project page click **Set commit hash**, paste the 40-character Git commit SHA and sign. The commit is stored on-chain and displayed alongside a link to the corresponding commit on the project's registered repository.
...
Badge weight is the default voting mode. Token-weighted proposals and NQG contract weight (for the SCF Public Goods project) are configured per proposal or per project — see [Membership](developers/membership.mdx).
