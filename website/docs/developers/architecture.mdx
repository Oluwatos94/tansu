...
Behind the scene, registration locks a **5 XLM collateral deposit** (via the configured collateral token) and checks that the project name is unique on-chain. Misuse or impersonation can be reported and addressed through governance, including collateral forfeiture.
...
- `ContractUpdated` - Collateral contract changed
